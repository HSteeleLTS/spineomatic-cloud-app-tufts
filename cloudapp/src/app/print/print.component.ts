        ...
        switch (detail) {
          ...
          case 'item_data.call_no_10':    
            ...
            if (detail.substring(18) - 1 < callNoVal.length) {
              ...
              var call_number_part =  this.getCallNoPart(callNoVal, detail.substring (18) - 1);  
			  if (detail == 'item_data.call_no_1' || (detail == 'item_data.call_no_2' && call_number_part.match(/^[A-Z]{3,}/))) {
				  
					
					call_number_part = call_number_part.replace(/^([A-Z]+)(\d+)(.*)$/g, "$1 <BR> $2$3");
					//call_number_part = call_number_part.replace(/^([A-Z]+\s\<br\>\s[0-9]+)(\.)([A-Z0-9]+)$/g, "$1 <br> $2$3");
					//alert(call_number_part);
					//return                          str.replace(new RegExp(`(?![^\\n]{1,${max}}$)([^\\n]{1,${max}})\\s`, 'g'), '$1' + br);
					
              }	
			  call_number_part = call_number_part.replace(/^(.*?)(\.)([A-Z]+)(.*)$/g, "$1 <BR> $2$3$4");
/* 			  if (/[^<][B\.([A-Z])/.test(call_number_part)) {
				  
				     call_number_part = call_number_part.replace(new RegExp(`\.([A-Z])`, 'g'), '$1.<BR>$2'); 
			  } */
			  return call_number_part
